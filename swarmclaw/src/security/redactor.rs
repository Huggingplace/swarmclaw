...
static PATTERNS: &[(&str, &str)] = &[
    ...
    (r"xoxb-[0-9]{11,}-[0-9]{12,}-[a-zA-Z0-9]{24,}", "[REDACTED_SLACK_TOKEN]"),
    (r"mship_[a-f0-9-]{36}", "[REDACTED_MOTHERSHIP_KEY]"),
    (r"AIza[0-9A-Za-z-_]{35}", "[REDACTED_GOOGLE_API_KEY]"),
    (r"(?:[A-Za-z0-9+/]{4}){10,}(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?", "[REDACTED_BASE64_DATA]"),
    (r"ghp_[a-zA-Z0-9]{36}", "[REDACTED_GITHUB_TOKEN]"),
];

static REGEX_SET: Lazy<RegexSet> = Lazy::new(|| {
    RegexSet::new(PATTERNS.iter().map(|(p, _)| p)).expect("Failed to build RegexSet")
});
